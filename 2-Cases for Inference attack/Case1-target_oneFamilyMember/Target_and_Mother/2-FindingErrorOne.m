...
foundprob = xlsread('Mother Probabilities.xlsx'); %% the results from applying FindProbS.m
%%For independent assumption, we use "Mother Only Probabilities (Independent Assumption).xlsx"
save('foundprob.mat','foundprob');%convert the file
familySum = xlsread('Mother Only Sum.xlsx'); %% the query noisy results file.
%%If the adversary knows partial information about the sum, use the file
%%containing the noisy results and the partial information.
