...
ProbabilitiesFM7Stest = xlsread('ProbabilitiesFM7Stest.xlsx');%%the results from applying partitions and permutations code
save('ProbabilitiesFM7Stest.mat','ProbabilitiesFM7Stest');%convert the file to .mat
...
%%
W= zeros(size(ProbabilitiesFM7Stest,1), 10);

for j = 1: size(ProbabilitiesFM7Stest,1) %table for potential genotypes of a sum value(Father Mother Target Son Son Son Son Son Son Son)
    
    for k= 1: size(mendeltableFM, 1) %table for father and mother Mendel law (Father Mother Target Probability)
        if ProbabilitiesFM7Stest(j, 1) == mendeltableFM (k , 1) && ProbabilitiesFM7Stest(j, 2) == mendeltableFM (k , 2) && ProbabilitiesFM7Stest(j, 3) == mendeltableFM (k , 3)
            W(j, 1:10) = ProbabilitiesFM7Stest(j, :);
        end
    end
end
W = W(any(W ~= 0,2),:);
...
%%
y= zeros(size(M,1), 10);
for j = 1: size(M,1) %table for potential genotypes of a sum value(Father Mother Target Son Son Son Son Son Son Son)
    P = 0;
...
R= zeros(size(M,1), 2);
for j = 1: size(M,1)
    Ptotal = y(j,11)+ y1(j,4)+y1(j,5)+ y1(j,6)+y1(j,7)+y1(j,8)+y1(j,9)+y1(j,10);
    Parent = y(j,3);
    R(j,1) = Parent;
    R(j,2) = Ptotal;
end
