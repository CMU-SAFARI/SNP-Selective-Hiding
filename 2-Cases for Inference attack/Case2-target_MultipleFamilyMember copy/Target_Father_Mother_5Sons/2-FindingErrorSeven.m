...
foundprob = xlsread('Father Mother Five Sons Probabilities.xlsx');%% the results from applying FindProbFM.m
%%For independent assumption, we use "Father Mother Five Sons Probabilities(Independent Assumption).xlsx"
save('foundprob.mat','foundprob');%convert the file 
familySum = xlsread('Father and Mother and Five Sons Sum.xlsx Sum.xlsx');%% the query noisy results file.
...
for k = 1:13
    ...
    for i = 1: size(unknown,1)
		...
		elseif familySum(i,k) == 14
			errorsum = errorsum + foundprob(15,j+1) .* double(abs(int32(j)-data(unknown(i,1),unknown(i,2))));
        elseif familySum(i,k) == 15
			errorsum = errorsum + foundprob(16,j+1) .* double(abs(int32(j)-data(unknown(i,1),unknown(i,2))));
		elseif familySum(i,k) == 16
			errorsum = errorsum + foundprob(17,j+1) .* double(abs(int32(j)-data(unknown(i,1),unknown(i,2))));
        else
			errorsum = errorsum + foundprob(18,j+1) .* double(abs(int32(j)-data(unknown(i,1),unknown(i,2))));
        end
        end
