...
FLeakedInfo = xlsread('FM3SLeakedInfo.xlsx');
save('FLeakedInfo.mat','FLeakedInfo');%convert the file
familySum = xlsread('Father Mother Three Sons Sum Partial.xlsx'); %% the query noisy results file which contains the partial info the adversary could know.
...
for k = 1:2:26
    ...
    for i = 1: 100
        ...
        elseif familySum(i,k) == 6
            ...
            if isnan(familySum(i,k+1))
                Comparesnp = FLeakedInfo(14, 3)- data(unknown(i,1),unknown(i,2)) ;
            else
                if familySum(i,k+1)== 0
                    x=0;
                elseif familySum(i,k+1)== 10
