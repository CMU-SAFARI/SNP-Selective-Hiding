...
ProbabilitiesFM3Stest = xlsread('ProbabilitiesFM3Stest.xlsx');%%the results from applying partitions and permutations code
save('ProbabilitiesFM3Stest.mat','ProbabilitiesFM3Stest');%convert the file to .mat
...
%%
W= zeros(size(ProbabilitiesFM3Stest,1), 6);

for j = 1: size(ProbabilitiesFM3Stest,1)  %table for potential genotypes of a sum value(Father Mother Target Son Son Son)
    for k= 1: size(mendeltableFM, 1)  %table for father and mother Mendel law (Father Mother Target Probability)
        if ProbabilitiesFM3Stest(j, 1) == mendeltableFM (k , 1) && ProbabilitiesFM3Stest(j, 2) == mendeltableFM (k , 2) && ProbabilitiesFM3Stest(j, 3) == mendeltableFM (k , 3)
            W(j, 1:6) = ProbabilitiesFM3Stest(j, :);
        end
    end
end
W = W(any(W ~= 0,2),:);
...
R= zeros(size(M,1), 2);
for j = 1: size(M,1)
    Ptotal = y(j,7)+ y1(j,4)+y1(j,5)+ y1(j,6);
    Parent = y(j,3);
    R(j,1) = Parent;
    R(j,2) = Ptotal;
end
