...
FLeakedInfo = xlsread('FMLeakedInfo.xlsx'); %the leaked SNPs values according to the probabilities of genotypes sum.
save('FLeakedInfo.mat','FLeakedInfo');%convert the file
familySum = xlsread('Father and Mother Sum Partial.xlsx');  %% the query noisy results file which contains the partial info the adversary could know.
...
for k = 1:2:26
    ...
    for i=1:100
        if ~isnan(familySum(i,k+1))
            if k==1  || k==3  || k==5 && (familySum(i,k) == 5 || familySum(i,k) == 6)
                if familySum(i,k+1)== 3
                    x=2;
                end
                Comparesnp = x - data(unknown(i,1),unknown(i,2))  ;
                n(i,k) = Comparesnp;
